...
export default function PreApprovalsPage() {
  ...
  const { data = [], isLoading, error, refetch } = useQuery({ queryKey: ["pre-approvals", status, sla], queryFn: () => api.get(`/pre-approvals?${params}`).then(response => response.data) });
  ...
  return (
    <div>
      ...
      <FilterBar><label htmlFor="status-filter" className="text-xs font-semibold text-slate-600">Status</label><select id="status-filter" value={status} onChange={event => setStatus(event.target.value)} className="input w-full sm:w-56"><option value="">Todos os status</option>{STATUS_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><label htmlFor="sla-filter" className="text-xs font-semibold text-slate-600">SLA</label><select id="sla-filter" value={sla} onChange={event => setSla(event.target.value)} className="input w-full sm:w-44"><option value="">Todos</option><option value="ON_TIME">No prazo</option><option value="DUE_SOON">Vence em breve</option><option value="OVERDUE">Vencida</option><option value="COMPLETED">Concluída</option></select><span className="ml-auto text-xs tabular-nums text-slate-500">{data.length} resultados</span></FilterBar>
